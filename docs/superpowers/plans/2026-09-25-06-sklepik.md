...
**Baza (jeden plik migracji, stosowany raz):**

| Plik | Odpowiada za |
|---|---|
| `supabase/migrations/20260925120000_sklepik.sql` | typy, cztery tabele, widok kroniki, RLS, publikacja realtime, `is_captain`, `kup_z_polki`, `wydaj_zamowienie`, `anuluj_zamowienie`, podmiana `review_bingo`, zasiew półki |

Migracja jest jedna, bo tabele, funkcje i polityki nie mają sensu osobno —
połowa tego zestawu w bazie to stan, w którym nic nie działa. Taski 1–5 dopisują
do **tego samego pliku**, każdy ze swoim commitem.
...
**Files:**
- Modify: `supabase/migrations/20260925120000_sklepik.sql` (dopisz na końcu)
...
Dopisz na końcu `supabase/migrations/20260925120000_sklepik.sql`:
...
```bash
git add supabase/migrations/20260925120000_sklepik.sql tests/db/sklepik.test.ts
...
w migracji z Taska 2 — pamiętaj o `db push` do obu projektów po każdej zmianie.
...
Dopisz na końcu `supabase/migrations/20260925120000_sklepik.sql`. Całe ciało
...
```bash
git add supabase/migrations/20260925120000_sklepik.sql tests/db/sklepik-blogoslawienstwo.test.ts
